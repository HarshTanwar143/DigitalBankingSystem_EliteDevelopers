...
public abstract class SecureControllerBase : Controller
{
    ...
    private static string? TryExtractMessage(string body)
    {
        ...
        try
        {
            ...
            if (root.TryGetProperty("message", out var m1) && !string.IsNullOrWhiteSpace(m1.GetString()))
                return m1.GetString();
            if (root.TryGetProperty("Message", out var m2) && !string.IsNullOrWhiteSpace(m2.GetString()))
                return m2.GetString();

            if (root.TryGetProperty("detail", out var d1) && !string.IsNullOrWhiteSpace(d1.GetString()))
                return d1.GetString();
            if (root.TryGetProperty("Detail", out var d2) && !string.IsNullOrWhiteSpace(d2.GetString()))
                return d2.GetString();

            if (root.TryGetProperty("title", out var t1) && !string.IsNullOrWhiteSpace(t1.GetString()))
                return t1.GetString();
            if (root.TryGetProperty("Title", out var t2) && !string.IsNullOrWhiteSpace(t2.GetString()))
                return t2.GetString();
